...
module.exports.fixerRendezVous=async(req,res)=>{
    try{
        const {dateRV,heureRV, patientId, medecinId,statutRV}=req.body
...
module.exports.getAllRVByPatient = async(req,res)=>{
    try{
        ...
        const RV = await rendezvousModel.find({patient: patient.id})
            if(!RV){
                throw new Error("aucun rendez-vous prevu");
            }
        res.status(200).json(RV)
...
module.exports.getAllRVByMedecin = async(req,res)=>{
    try{
        ...
        const RV = await rendezvousModel.find({medecin: medecin.id})
            if(!RV){
                throw new Error("aucun rendez-vous prevu");
            }
        res.status(200).json(RV)
...
module.exports.getCreneauxDispos = async(req,res)=>{
    try{
        ...
        const RV = await rendezvousModel.find({medecin: medecin.id,date})
        if(!RV){
            throw new Error("aucun rendez-vous prevu");
        }

        const creneauxPris = RV.map(rv => rv.heureRV);
